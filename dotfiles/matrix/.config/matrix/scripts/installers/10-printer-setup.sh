...
sudo cp "$HOME/.config/matrix/scripts/system-configs/matrix-cups-sysusers.conf" /etc/sysusers.d/
sudo systemd-sysusers

# 3. Apply Systemd sandboxing
echo " -> Applying strict kernel sandboxing to CUPS..."
sudo mkdir -p /etc/systemd/system/cups-browsed.service.d/
echo "  -> Copying matrix-cups-browsed-systemd.conf to /etc/systemd/system/cups-browsed.service.d/10-matrix.conf"
sudo cp "$HOME/.config/matrix/scripts/system-configs/matrix-cups-browsed-systemd.conf" /etc/systemd/system/cups-browsed.service.d/10-matrix.conf
...
sudo bash -c 'cat << EOF > /etc/cups/cups-browsed.conf
...
if sudo grep -q "^SystemGroup" /etc/cups/cups-files.conf; then
    echo "  -> Modifying existing SystemGroup in /etc/cups/cups-files.conf"
    sudo sed -i 's/^SystemGroup.*/SystemGroup cups-browsed sys root/' /etc/cups/cups-files.conf
else
    echo "  -> Appending SystemGroup cups-browsed sys root to /etc/cups/cups-files.conf"
    echo "SystemGroup cups-browsed sys root" | sudo tee -a /etc/cups/cups-files.conf > /dev/null
fi
