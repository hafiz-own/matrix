...
# DESC: Spawns the GUI overlay for setting new reminders.
...
Spawns the GUI overlay for setting new reminders.
...
# Prompt user via Walker dmenu
input=$(echo "" | walker -d --placeholder "Set Reminder (e.g., 15 Take pizza out)")
...
if [[ -z "$message" || "$message" == "$minutes" ]]; then
